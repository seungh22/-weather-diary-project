...
public class DiaryController {
    ...
    void createDiary(@RequestParam @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) @ApiParam(value = "일기 저장할 날짜", example = "2020-02-02") LocalDate date, @RequestBody String text) {
        diaryService.createDiary(date, text);
    }

    @ApiOperation(value = "선택한 날짜의 모든 일기 데이터를 가져옵니다")
    @GetMapping("/read/diary")
    List<Diary> readDiary(@RequestParam @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) @ApiParam(value = "일기 데이터를 가져올 날짜", example = "2020-02-02") LocalDate date) {
    ...
    @ApiOperation(value = "선택한 날짜의 첫번째 일기의 텍스트를 수정합니다")
    @PutMapping("/update/diary")
    void updateDiary(@RequestParam @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) @ApiParam(value = "일기 수정할 날짜", example = "2020-02-02") LocalDate date, @RequestBody String text) {
